...
namespace NLayer.Teacher.ViewComponents
{
    public class StudentTotalExamRateViewComponent : ViewComponent
    {
        ...
        public StudentTotalExamRateViewComponent(UserManager<AppUser> userManager, IScorsService scorsService, IMapper mapper)
        {
        ...
        {
            var scorslist =_mapper.Map<List<Scors>>(await _scorsService.GetTogetherList());
            ...
            {
                TotalBiyoloji = biyolojiScores.Any() ? Math.Round(biyolojiScores.Sum(x => x.Average) / biyolojiScores.Count() * 100) : 0,
                TotalCografya = cografyaScores.Any() ? Math.Round(cografyaScores.Sum(x => x.Average) / cografyaScores.Count() * 100) : 0,
